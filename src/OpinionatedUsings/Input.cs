...
namespace OpinionatedUsings
{
    public static class Input
    {
        ...
        /// <param name="excludes">
        /// GLOB patterns to exclude files matching patterns
        /// </param>
                ...
                if (Path.IsPathRooted(pattern))
                {
                    ...
                    if (root == null)
                    {
                        throw new ArgumentException(
                            $"Root could not be retrieved " +
                            $"from rooted pattern: {pattern}");
                    }
                ...
                List<string> accepted =
                    files
                        .Where((path) =>
                        {
                            ...
                            return globExcludes.TrueForAll(
                                glob => !glob.IsMatch(rootedPath));
                        })
                        .ToList();
