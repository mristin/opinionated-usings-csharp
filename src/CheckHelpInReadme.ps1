...
function Main
{
    ...
    $program = Join-Path $buildDir "OpinionatedUsings"
    if (!(Test-Path $program))
    {
        $program = Join-Path $buildDir "OpinionatedUsings.exe"

        if (!(Test-Path $program))
        {
            throw ("The program could not be found: $program; " +
                    "did you `dotnet publish -c Release -o $buildDir`?")
        }
    }
